              ...
              unsigned char psize, unsigned char minc, unsigned char circ, unsigned char dir)
{
    ...
    SET_BIT((DMA->dma_channel[channel_num].CCR), (priority_level << 12)); 
    
    //设置存储器数据宽度
    CLEAN_BIT((DMA->dma_channel[channel_num].CCR), (0x3 << 10)); 
    SET_BIT((DMA->dma_channel[channel_num].CCR), (msize << 10)); 
    
    //设置外设数据宽度
    CLEAN_BIT((DMA->dma_channel[channel_num].CCR), (0x3 << 8));
    SET_BIT((DMA->dma_channel[channel_num].CCR), (psize << 8)); 
    
    //设置存储器地址增量模式
    SET_BIT((DMA->dma_channel[channel_num].CCR), (minc << 7)); 
    //设置数据传输方向
    SET_BIT((DMA->dma_channel[channel_num].CCR), (dir << 4)); 
    //设置循环模式 
    SET_BIT((DMA->dma_channel[channel_num].CCR), (circ << 5));
}
...
{   
    //数据传输数量，范围为0～65535
    DMA->dma_channel[channel_num].CNDTR = data_size;        
    //外设数据寄存器的基地址，作为数据传输的源或目标
    DMA->dma_channel[channel_num].CPAR  = target_address;   
    //存储器地址，作为数据传输的源或目标 
    DMA->dma_channel[channel_num].CMAR  = data_address;     
    
    //通道使能
    SET_BIT((DMA->dma_channel[channel_num].CCR), (1 << 0)); 
}