...
{
    //interrupts右移2位相当于处以4
    CLEAN_BIT((nvic->IPR[interrupts >> 2]), (0xF << (interrupts % 4) * 8));                  
    //pri_num与0xF作运算防止传入超过15的数时污染相邻的中断位
    SET_BIT((nvic->IPR[interrupts >> 2]), (((pri_num & 0xF) << 4) << (interrupts % 4) * 8)); 
}